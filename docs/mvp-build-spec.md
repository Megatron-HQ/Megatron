...
installPath, projectPath, version, installedAt, lastUpdated }] } }` — a wrapper, not a flat map;
  unwrap `.plugins`. `installPath` contains a nested `skills/` dir. `version` can literally be the
  string `"unknown"` (non-semver). `scope` is one of `user` / `project` / `local` — **three**
  values, not two; `project` and `local` additionally carry `projectPath`, and `installPath` is
  version-addressed so it does **not** uniquely identify an install. See "Plugin install identity
  and project scope" in `docs/data-model.md`.
...
| Cost analytics, MCP dashboard, knowledge graph, journal, coach, marketplace | Out of scope for this MVP entirely. **Phase 2a is now in progress** — see `docs/usage-analytics.md`: the "Usage" section + its **Activity** retrospective (`history.jsonl`) landed 2026-09-05 (PR1); Cost / Skills-association / Model & effort / Resident-tax are PR2–5, still design-only                                                                                                                                                                                                                                                                                                                                                                   | Per original roadmap's Phase 2+ — underway                                                                                            |
...
| Acting on a project/local install whose folder isn't granted                | Project-scope actions (2026-09-03) run the CLI from the install's own `project_path`, but only once that folder is granted. Ungranted, the project's settings are unreadable, so the install's status shows as **Unknown** and Enable/Disable would be toggling a switch whose position Megatron can't read; the buttons stay disabled and link to Manage Folders. Lifting this means either acting blind or finding a way to read a project's `enabledPlugins` outside the grant model — neither is obviously right | Granting the folder proves to be friction users resent for an action they've already confirmed                                        |
