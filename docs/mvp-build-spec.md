...
- **Correction to the "not recoverable" call on explicit-vs-autonomous invocation** (this was
  ...
  Three real, distinguishable shapes were found in actual transcripts: a harness-parsed
  `<command-name>/skill-name</command-name><command-args>...</command-args>` structure; a bare
  `/skill-name` sitting inside otherwise free-form prose (not the structured form — the
  harness only produces the structured form when the command is the literal start of the
  message); and no mention at all (the assistant invoked the skill on its own, matching
  context to the skill's own `description`, confirmed against a real `Portfolio/CLAUDE.md`
  rule mandating `visual-verify` after CSS changes). `skill_invocations.trigger_type` (added
  below) encodes
  this as `'harness_command' | 'text_mention' | 'autonomous'`, derived deterministically via
  string/regex matching — no LLM call, consistent with the rest of this app's no-API-key
  posture. See the new Invocation trigger classification section.
...
### Invocation trigger classification (added post-M1)

M1 shipped, then a real-data investigation of `NULL` `args_text` rows (see Revisions above)
found that invocation _origin_ — harness slash-command, a bare mention in free text, or fully
autonomous — is recoverable from the **nearest preceding user-role message with string
content**, even though it isn't recoverable from the `Skill` tool_use block itself. Algorithm:

1. Preceding message matches `<command-name>/<skill-name></command-name>` (skill name must
   match) → `harness_command`.
2. Else, preceding message contains `/<skill-name>` as a substring, word-boundary-checked (so
   `/grill-mean` doesn't false-match `/grill-me`) → `text_mention`.
3. Else → `autonomous`.
...
**Implemented and verified.** 90 tests total (11 new), full lint/typecheck/test gate green.
Real-data spot check after a fresh scan: 14 `autonomous`, 29 `text_mention`, **0
`harness_command`** — confirmed by an exhaustive sweep across every transcript on this
machine, not just a sample: there is exactly one (harness-tag → later same-skill invocation)
pair in the entire dataset, and it has 41 intervening user turns between them, not zero. This
means a session-_starting_ slash command does not itself produce an immediately-following
`Skill` tool_use block — starting a session via `/skill-name` and explicitly invoking the
`Skill` tool mid-conversation are evidently different mechanisms, the former apparently
loading the skill's instructions without a distinguishable tool-call event for this scanner to
see. `harness_command` is real, correctly implemented, and unit-tested against a synthetic
fixture — it's just legitimately unobserved in this user's real history so far, because it
requires a _mid-session_ literal `/skill-name` re-invocation with nothing in between, which
hasn't happened yet. Not a bug; a fact about the data, discovered rather than assumed (an
earlier draft of this section confidently cited a specific real example that turned out to be
wrong once actually checked — corrected in Revisions above).
...
| ------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------- |
| In-app skill editing                                                                                                | Only write surface; a bug here corrupts a file Claude Code executes                                                                                                                                                                                                                                                                                                                                                           | Read-only inventory shows repeat weekly use                                                           |
