...
| Disabled-skill detection (added 2026-08-21) | `scanSkills()`/`scanPluginRegistry()` now also read `~/.claude/settings.json` (added to `permissions.ts`'s `TIER_1_FILES`) via `claude-settings.ts`. A Plugin skill's `disabled_reason` comes from `enabledPlugins`, resolved at **user scope only**. A Global/Project skill's comes from `skillOverrides` (`off` only — `name-only`/`user-invocable-only` don't unload the skill, so they're left `NULL`); a Project skill gets the full `local > project > user` merge, a Global skill resolves **user scope only**. | Verified against [discover-plugins.md](https://code.claude.com/docs/en/discover-plugins.md), [skills.md](https://code.claude.com/docs/en/skills.md), and [settings-reference.md](https://code.claude.com/docs/en/settings-reference.md), not assumed. The user-scope-only restriction for Plugin/Global skills isn't a fidelity shortcut — it's a real constraint: those rows have `project_root = NULL` (one row total), so they structurally cannot represent "disabled in project X only" without a second scan dimension. A Project skill's row is already anchored to one `project_root`, so it can absorb the full per-project merge. See `docs/data-model.md`'s `disabled_reason` entry. |
