...
Exactly one stamp of color breaks that quiet: acid lime, reserved for the single active thing on screen — the selected sidebar filter, or a primary call-to-action. A separate, purely functional palette of red/amber/green flags reports lint state, and never borrows the stamp's job or vice versa. Everything else — borders, hover states, focus rings — is drawn in ink and rule-lines, the ledger's native materials.
...
Near-monochrome ledger ink on paper, with exactly two functional color layers on top: one accent for "this is the active line," and three status flags for lint state. Nothing else in the system carries color. Built on shadcn's `new-york`/`neutral` base (`components.json`) before the ledger's own accent and flags are layered on top.
...
**The Flags-Aren't-The-Stamp Rule.** Red/amber/green report lint state only. They never substitute for the lime accent, and a status is never color alone — always paired with an icon and a label.
...
### Don't:
- **Don't** give source tiers (global/project/plugin) their own color family — icon + neutral badge carries that meaning; color stays reserved for lint status and the lime accent.
