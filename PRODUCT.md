...
Inventories, lints, and tracks usage of every Claude Code skill a user has — global, project, and plugin-installed — read-only, deterministic, single-purpose in v1. Success is a user trusting the inventory enough to open it repeatedly, not just once at install, to catch broken or shadowed skills before Claude Code hits them and to see which skills earn their context budget.
...
- Read-only, deterministic, single-purpose — no speculative write paths or LLM calls in the core product.
